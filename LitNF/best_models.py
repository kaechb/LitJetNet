...
def train(config, hyperopt=False, load_ckpt=None,i=0,root=None):
    ...
    print(model.config)
    callbacks = [ModelCheckpoint(monitor="val_logprob",save_top_k=50, filename='{epoch}-{val_logprob:.2f}-{val_w1m:.4f}-{val_w1efp:.6f}-{val_fpnd:.2f}',every_n_epochs=100) ]
...
if __name__ == "__main__":
    ...
    for index,row in df.iterrows():
        ...
        root="/beegfs/desy/user/kaechben/bestmodels_final/top_{}{}_{}".format(typ,c,p)
        if not hyperopt:
