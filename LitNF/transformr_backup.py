...
class Block(nn.Module):
    def __init__(self, embed_dim=128, num_heads=8, ffn_ratio=4,
        ...
        self.ffn_dim = self.embed_dim * ffn_ratio
...
class ParticleTransformerCritic(nn.Module):
    ...
    def __init__(self,
        ...
        self._counter = 0
        embed_dims =  list(num_heads*np.array(embed_dims).astype(int))
    ...
    def forward(self, x, v=None, mask=None):
        ...
        # mask: (N, 1, P) -- real particle = 0, padded = 1
...
class Disc(nn.Module):
    def __init__(self,config):
        ...
        self.encoder =ParticleTransformerCritic(self.n_dim,num_heads=config["heads"],num_classes=1,num_layers=config["num_layers"])
    ...
    def forward(self, x, m=None,p=None, mask=None,noise=0):
        ...
        scores = self.encoder(x.permute(0,2,1), mask=mask.bool())#,v.permute(0,2,1)
        # assert (v==v).all()
        assert (scores==scores).all()
...
class Gen(nn.Module):
    def __init__(self,config):
        ...
        self.n_part =config["n_part"]
        self.embbed =nn.Linear(self.n_dim, self.l_dim)
        self.encoder =ParticleTransformerGen(self.n_dim,num_heads=config["heads"],num_layers=config["num_layers"])
       
    def forward(self, x, mask=None):
        #x = x.transpose(0,1)
        # v = canonical(x)
        # assert (v==v).all()
        x = self.encoder(x.permute(0,2,1), mask=mask.bool())#,v.permute(0,2,1)
        assert (x==x).all()
