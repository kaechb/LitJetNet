...
class TransGan(pl.LightningModule):
    def create_resnet(self, in_features, out_features):
        ...
        return nets.ResidualNet(
            ...
            if "batchnorm" in self.config.keys()
            else 0,
        )
    ...
    def sampleandscale(self, batch, scale=False):
        ...
        if scale:
            ...
            z_scaled = self.data_module.scaler.inverse_transform(
                z.reshape(len(batch), self.n_dim * self.n_part)
            )
    ...
    def configure_optimizers(self):
        ...
        lr_scheduler_nf = (
            None
            if not self.config["sched"]
            else CosineWarmupScheduler(
                opt_nf, warmup=1, max_iters=10000000 * self.config["freq"]
            )
        )
    ...
    def compute_gradient_penalty(self, D, real_samples, fake_samples, phi):
        ...
        if self.config["mass"]:
            m = mass(
                interpolates.reshape(
                    len(real_samples), self.n_part * self.n_dim
                ).detach()
            )
            d_interpolates = D.train()(
                interpolates.requires_grad_(True), m.requires_grad_(True)
            )
        else:
    ...
    def training_step(self, batch, batch_idx):
        ...
        nf_loss -= (
            self.flow.to(self.device).log_prob(batch).mean()
        )  # c if self.config["context_features"] else None
        nf_loss /= self.n_dim * self.n_part
        opt_nf.zero_grad()
        self.manual_backward(nf_loss)
        opt_nf.step()
        self.log(
            "logprob", nf_loss, on_step=True, on_epoch=False, prog_bar=True, logger=True
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        m_gen = mass(
            z_scaled[:, : self.n_dim * self.n_part], self.config["canonical"]
        ).cpu()
        m_t = mass(
            true_scaled[:, : self.n_dim * self.n_part], self.config["canonical"]
        ).cpu()
        ...
        cov, mmd = cov_mmd(
            fake_scaled.reshape(-1, self.n_part, self.n_dim),
            true_scaled.reshape(-1, self.n_part, self.n_dim),
            use_tqdm=False,
        )
        try:
            fpndv = fpnd(
                fake_scaled.reshape(-1, self.n_part, self.n_dim).numpy(),
                use_tqdm=False,
        ...
        self.metrics["val_w1p"].append(
            w1p(
                fake_scaled.reshape(len(batch), self.n_part, self.n_dim),
                true_scaled.reshape(len(batch), self.n_part, self.n_dim),
            )
        )
        w1m_ = w1m(
            fake_scaled.reshape(len(batch), self.n_part, self.n_dim),
        ...
        temp = {
            ...
            "val_mmd": mmd,
            "val_cov": cov,
            "val_w1m": self.metrics["val_w1m"][-1][0],
            "val_w1efp": self.metrics["val_w1efp"][-1][0],
            "val_w1p": self.metrics["val_w1p"][-1][0],
