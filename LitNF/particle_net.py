...
def pad_array(jagged_array, max_len, value=0.0, dtype="float32"):
    rectangluar_array = np.full(
        shape=(len(jagged_array), max_len), fill_value=value, dtype=dtype
    )
    ...
    def __init__(self, settings):
        ...
        for layer_idx, layer_param in enumerate(settings["conv_params"]):
            ...
            self.conv_process.append(
                ParticleDynamicEdgeConv(previous_output_shape, channels, k=K).to(DEVICE)
            )
