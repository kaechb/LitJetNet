...
if __name__ == "__main__":
    ...
    if not hyperopt:
        ...
        for i in range(num_samples):
            ...
            config["network_layers"]=np.random.randint(3, 6)
            config["network_nodes"]= np.random.randint(64, 300)
            config["coupling_layers"]= np.random.randint(5, 20)
            config["lr"]= stats.loguniform.rvs(0.0001, 0.001,size=1)[0]
            config["batchnorm"]= np.random.choice([True])
            config["n_mse_turnoff"]= np.random.choice([5000,10000])
            
            config["bins"]= np.random.randint(4, 7)
            ...
            # config["spline"]= np.random.choice([True])
            config["context_features"]= 2
