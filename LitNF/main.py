...
if __name__ == "__main__":
    ...
    delete=['autoreg',"disc","fc","quantile","bullshitbingo","bullshitbingo2","coupling_layers","batchnorm","lr_nf"]
    for key in delete:
        config.pop(key, None)
    hyperopt=True
    config["val_check"]=50
    ...
    if hyperopt:
        ...
        config["liverpool"]=np.random.choice([True,False])
        ...
        config["opt"]=np.random.choice(["AdamW","Adam","RMSprop"])#
        config["lr_g"]=np.random.choice([0.00005,0.00001])  
        config["ratio"]=np.random.choice([0.9,1,1.1,])
        config["num_layers"]=np.random.choice([4])#5,6,7
        config["weirdgloss"]=np.random.choice([False])
        if config["num_layers"]>6:
            config["hidden"]=np.random.choice([256,128])
            config["l_dim"]=np.random.choice([10,20])
        else:
            config["l_dim"]=np.random.choice([25,30,50])
            config["hidden"]=np.random.choice([1024,1500,2048])
        config["heads"]=np.random.choice([2,4])
        config["val_check"]=10
