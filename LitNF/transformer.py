...
class ParticleTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.embed = Embed(input_dim, embed_dims, activation=activation,normalize_input=not self.gen )
        ...
        if not self.gen:
            ...
            self.out=nn.Linear(embed_dims[-1],input_dim)
...
class Disc(nn.Module):
    def __init__(self,config):
        ...
        self.encoder =ParticleTransformer(self.n_dim,num_heads=config["heads"],num_classes=1,num_layers=config["num_layers"],embed_dims=config["embed_dims"],gen=False)
    ...
    def forward(self, x, m=None,p=None, mask=None,noise=0):
        ...
        scores = self.encoder(x.permute(0,2,1), mask=~mask.bool())#,v.permute(0,2,1),v.permute(0,2,1)
        # assert (v==v).all()
...
class Gen(nn.Module):
    def __init__(self,config):
        ...
        self.l_dim =config["l_dim"]*config["heads"]
        self.n_part =config["n_part"]
        self.encoder =ParticleTransformer(self.n_dim,num_heads=config["heads"],num_layers=config["num_layers"],embed_dims=config["embed_dims"],gen=True)
        
    def forward(self, x, mask=None):
        #x = x.transpose(0,1)
        # v = canonical(x)
        # assert (v==v).all()
        x = self.encoder(x.permute(0,2,1), mask=~mask.bool())#,v.permute(0,2,1), v.permute(0,2,1)
        assert (x==x).all()
        x=x.permute(1,0,2)
        return x
